...
Judge observable behavior and room impact, not intent.

Incivility is about harm to a person, not about register. Rudeness, bluntness, mockery, profanity, sarcasm, teasing and dismissiveness are not incivility. Criticism of a thing is never incivility, however harsh: work, code, projects, designs, tools, artwork and decisions may be called bad, broken, lazy or worthless without limit. The bar for incivility or a personal attack is sustained hostility aimed at a specific person that a reasonable participant would experience as an attack rather than as banter between willing participants.

Apply that bar strictly, and default to allow. This room is informal and its members are capable of answering rudeness themselves. A moderator that polices tone costs the room more than the rudeness does, so a wrong call here is worse than a missed one. If a message could plausibly be read as banter, allow it. If the hostility is a single remark rather than a pattern, allow it. If you are weighing whether something crossed the line, it did not.
